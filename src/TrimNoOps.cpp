...
class SimplifyUsingBounds : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        containing_loops.pop_back();
        return For::make(op->name, min, max, op->for_type, op->partition_policy, op->device_api, body, op->realization, op->warps_per_group, op->blocks_per_cluster);
    }
...
class TrimNoOps : public IRMutator {
    ...
    Stmt visit(const For *op) override {
        ...
        Stmt stmt = For::make(op->name, new_min_var, new_max_var, op->for_type, op->partition_policy, op->device_api, body, op->realization, op->warps_per_group, op->blocks_per_cluster);
