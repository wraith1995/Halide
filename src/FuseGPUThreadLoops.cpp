...
class LowerAsyncCompletions : public IRMutator {
    ...
    Expr visit(const Call *op) override {
        ...
        if (op->is_intrinsic(Call::async_wait)) {
            ...
            if ((CompletionKind)*kind == CompletionKind::CpAsyncBulk) {
                // async_wait(CpAsyncBulk, scope, token, parity): every consumer thread waits the
                // mbarrier phase; completion = the bytes have landed.
                internal_assert(op->args.size() == 4)
                    << "async_wait(CpAsyncBulk) expects (kind, scope, token, parity).\n";
