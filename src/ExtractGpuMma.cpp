...
struct GpuMma : public IRMutator {
    ...
    Stmt visit(const ProducerConsumer *op) override {
        ...
        if (op->is_producer) {
            ...
            // Each operand is loaded from its shared TILE by ONE ldmatrix — no per-lane
            // scalar fragment gather. The per-lane address is a tile ROW-START derived from
            // the tile geometry (base + strides), hence 16-byte aligned as ldmatrix requires.
            // The reduction dim k must be contiguous in shared (As k-contiguous via
            // reorder_storage; Bs k-contiguous by default), so the 8 halfs ldmatrix reads
            // are a k-run. Validated on A100: A = ldmatrix.x4 (16m×16k), B = ldmatrix.x2.trans
            // (16k×8n). `kb` is the absolute k base of the current k-tile.
            auto frag_load = [&](const string &op, Type ft, const string &nm, Buffer<> im,
                                 Parameter pm, const Expr &addr) {
                Expr ld = Load::make(Float(16), nm, addr, im, pm, const_true(), ModulusRemainder());
                return Call::make(ft, op, {ld}, Call::Intrinsic);
            };
            auto mma_step = [&](const Expr &kb) {
                // A 16m×16k tile, lane L -> row m=L%16, k col-block (L/16)*8 (aligned).
                Expr a_addr = base_a + (ln % 16) * da_m + (kb + (ln / 16) * 8) * da_k;
                Expr afrag = frag_load("gpu_ldmatrix_x4", Float(16, 8), a_name, a_image, a_param, a_addr);
                // B 16k×8n tile, lane L -> col n=L%8, k-block (L/8)*8 (aligned), .trans.
                Expr b_addr = base_b + (kb + (ln / 8) * 8) * db_k + (ln % 8) * db_n;
                Expr bfrag = frag_load("gpu_ldmatrix_x2_trans", Float(16, 4), b_name, b_image, b_param, b_addr);
