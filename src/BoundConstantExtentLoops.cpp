        ...
        if (op->for_type == ForType::Unrolled ||
            ...
            if (e == nullptr && permit_failed_unroll && op->for_type == ForType::Unrolled) {
                ...
                return For::make(op->name, op->min, op->max,
                                 ForType::Serial, op->partition_policy, op->device_api, std::move(body), op->realization, op->warps_per_group, op->blocks_per_cluster);
            }
            ...
            return For::make(op->name, op->min, (op->min + e) - 1,
                             op->for_type, op->partition_policy, op->device_api, std::move(body), op->realization, op->warps_per_group, op->blocks_per_cluster);
        } else {
            return IRMutator::visit(op);
        }
    }
