    ...
    std::tuple<Stmt, bool> visit_for(const For *op) {
        ...
        if (op->for_type == ForType::Serial && !op->body.as<Acquire>()) {
            stmt = For::make(op->name, min_steady, max_steady - 1,
                             op->for_type, op->partition_policy, op->device_api, simpler_body, op->realization, op->warps_per_group, op->blocks_per_cluster);

            if (make_prologue) {
                prologue = For::make(op->name, op->min, min_steady - 1,
                                     op->for_type, op->partition_policy, op->device_api, prologue, op->realization, op->warps_per_group, op->blocks_per_cluster);
                stmt = Block::make(prologue, stmt);
                mutated = true;
            }
            if (make_epilogue) {
                epilogue = For::make(op->name, max_steady, op->max,
                                     op->for_type, op->partition_policy, op->device_api, epilogue, op->realization, op->warps_per_group, op->blocks_per_cluster);
            ...
            stmt = For::make(op->name, op->min, op->max, op->for_type, op->partition_policy, op->device_api, stmt, op->realization, op->warps_per_group, op->blocks_per_cluster);
        }
...
class RenormalizeGPULoops : public IRMutator {
    ...
    Stmt visit(const LetStmt *op) override {
        ...
        if (f && in_gpu_loop && !in_thread_loop) {
            ...
            inner = For::make(f->name, f->min, f->max, f->for_type, f->partition_policy, f->device_api, inner, f->realization, f->warps_per_group, f->blocks_per_cluster);
            return mutate(inner);
    ...
    Stmt visit(const IfThenElse *op) override {
        ...
        if (allocate_a && allocate_b) {
            ...
            inner = For::make(for_a->name, for_a->min, for_a->max, for_a->for_type, for_a->partition_policy, for_a->device_api, inner, for_a->realization, for_a->warps_per_group, for_a->blocks_per_cluster);
            return mutate(inner);
        } else {
