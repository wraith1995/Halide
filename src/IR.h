...
struct Call : public ExprNode<Call> {
    ...
    enum IntrinsicOp {
        ...
        alloca,
        // Async-completion requirements for bulk/tensor memory movement + warp-group MMA, the
        // companions of sync_requirement (a rendezvous) for operations that COMPLETE asynchronously
        // (issue ... later wait). Target-independent; lowered by (CompletionKind x SyncScope x
        // target) at the same selector seam -- to cp.async commit/wait_group, an mbarrier
        // transaction (sm_90 cp.async/TMA), or wgmma.commit/wait_group. Unlike a barrier, the
        // mechanism may materialize STATE (an mbarrier), whose init->use visibility is a transitive
        // sync requirement generated by the barrier analysis (see research/fusegpu_rearch_plan.md
