...
Stmt Simplify::visit(const For *op) {
    ...
    if (is_no_op(new_body)) {
        ...
        return For::make(op->name, make_zero(Int(32)), shifted_max, op->for_type, op->partition_policy, op->device_api, new_body, op->realization, op->warps_per_group, op->blocks_per_cluster);
    } else if (op->min.same_as(new_min) &&
               op->max.same_as(new_max) &&
               op->body.same_as(new_body)) {
        return op;
    } else {
        return For::make(op->name, new_min, new_max, op->for_type, op->partition_policy, op->device_api, new_body, op->realization, op->warps_per_group, op->blocks_per_cluster);
    }
}
...
Stmt Simplify::visit(const Allocate *op) {
    ...
    if (body_if &&
        ...
        Stmt stmt = Allocate::make(op->name, op->type, op->memory_type,
                                   ...
                                   new_expr, op->free_function, op->padding, op->swizzle);
